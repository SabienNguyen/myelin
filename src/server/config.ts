...
export function loadConfig(path = process.env.HARNESS_CONFIG ?? './harness.config.json'): HarnessConfig {
  ...
  // The `claude-sdk:` subscription route no longer exists. Failing here, at boot, is deliberate:
  // models.ts would otherwise hand the id to the Anthropic API provider, and a machine with a key
  // configured would silently bill it for calls the user believed rode their subscription.
  const removedRoute = Object.entries(cfg.models)
