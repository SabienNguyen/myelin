...
import { gradeTone, normalizeContour, type Tone, type ToneGrade } from './toneContour.js';

export interface PronounceAttempt {
  grade: ToneGrade;
  /** The learner's contour in the same normalized space as TONE_TEMPLATES — for the overlay.
   *  null when there wasn't enough voiced sound to draw (grade.unscorable is then true too). */
  contour: number[] | null;
}

export function gradePronunciation(
  samples: Float32Array | number[], sampleRate: number, tone: Tone,
): PronounceAttempt {
  const f0 = pitchTrack(samples, sampleRate);
  return { grade: gradeTone(f0, tone), contour: normalizeContour(f0) };
}
