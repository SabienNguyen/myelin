...
function RuntimeInner(
  ...
  const { runtime, store } = useChatCoreRuntime({ mode, threadId, initialMessages: initial });
  ...
  return (
    // ChatStoreContext: the attach-aware composer's direct line to sendMessage (runtimeAdapter.ts
    // explains why attachments cannot ride assistant-ui's own send path).
