// Copy the bundled Engram into the packaged app's resources directory.
//
// This is an afterPack hook rather than an `extraResources` entry because electron-builder's copier
// STRIPS `node_modules` out of extra resources — and Engram's shipped server imports
...
// inside an archive, and the harness runs Engram as a stdio child process.
...
export default async function afterPack(context) {
  const from = join(root, 'vendor', 'engram');
  if (!existsSync(from)) {
    throw new Error('vendor/engram is missing — run `npm run bundle:engram` first.\n'
      + 'Shipping without it would produce an app that starts and then cannot remember anything.');
  }
  const to = join(context.appOutDir, 'resources', 'engram');
  ...
  for (const required of [entry, deps]) {
    if (!existsSync(required)) throw new Error(`bundled Engram is incomplete: missing ${required}`);
  }
  console.log(`  • bundled Engram  to=${to}`);
}
