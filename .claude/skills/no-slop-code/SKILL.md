...
**1. Loreweaver is the only writer of `pages/` and `students/`.** The harness reaches them
...
**2. Never parse vault markdown in harness code.** `Loreweaver.listSlugs()` deliberately reads
filenames only. Page structure is Loreweaver's business; ask it via `read_page`.
...
mirrored in `src/shared/loreweaver.ts`. These are documented divergence risks. Do not add a fourth
...
real Loreweaver process, then asserts the evidence landed in the student's file on disk.
...
- Adding a dependency needs a reason in the commit body. Loreweaver next door ships three.
