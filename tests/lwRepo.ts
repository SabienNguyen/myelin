...
// The sibling engram checkout the integration tests spawn as the real MCP server.
//
// This used to be `${HOME}/Dev/personal/engram` in every test file, which forced anyone whose
...
// — makes the whole suite run wherever it sits, the same layout resolveEngram() already falls
...
const sibling = join(HARNESS_ROOT, '..', 'engram');

export const LW_REPO = existsSync(sibling)
  ? sibling
  : `${process.env.HOME}/Dev/personal/engram`;
